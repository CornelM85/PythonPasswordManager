...
class LoginsFrame(ctk.CTkFrame):
    ...
    def add_site_url(self):
        ...
        if url != 'Not a login page!':
            self.add_site_info_to_db()
            # browser.get_site_icon(url)
            # image = ctk.CTkImage(Image.open(resource_path(f'Images/{browser.set_icon_name(url)}')), size=(15, 15))
            # icon = ctk.CTkLabel(self.web_sites, image=image, width=20, height=10, text='')
            # icon.grid(row=entries + 1, column=0, padx=(0, 15))
        ...
        with open('db.json', 'r') as f:
            ...
            for dictionary in values:
                for k, v in dictionary.items():
                    ...
                    if k == 'url':
                        url = ctk.CTkLabel(self.web_sites, height=10, width=630, text=v, text_color='#00A2E8',
